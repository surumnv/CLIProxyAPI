...
This fork adds the following changes on top of upstream. The core goal is to keep requests forwarded from real Codex / Claude Desktop clients as client-faithful as possible (instead of being rewritten or stripped by the proxy), aligning the HTTP headers, HTTP/1.1 wire format, and TLS fingerprint with the genuine client where practical, plus a few management-panel and provider-config improvements. Grouped by feature:

- **Faithful inbound header and key-field passthrough.** Headers sent by the real Codex / Claude Desktop client are preserved and forwarded upstream as-is, instead of being dropped by a strict allowlist. The proxy still keeps authoritative control of the headers it must own (`Authorization` / `X-Api-Key` carry the provider credential and are never leaked upstream; `Anthropic-Beta`, `Accept`, connection/length/encoding headers are managed by the proxy). Credential- and session-scoped headers (`Cookie`, `Proxy-Authorization`, `Expect`, `Content-Encoding`) are never forwarded. Claude upstream requests keep the client-supplied `temperature`; third-party Claude bases no longer get OAuth-only betas injected; when a Responses request is translated to Chat Completions, Responses-Lite-only headers are stripped so the upstream is not rejected for a protocol mismatch.
- **Original HTTP/1.1 header order preservation, with source-specific name casing.** Outbound HTTP/1.1 requests are first written in the header **order** observed from the inbound client, instead of Go's default alphabetical ordering. This covers the main Codex / Claude paths and the OpenAI-compatible provider forwarding path. Backed by a raw HTTP/1.1 transport with a keep-alive connection pool (idle timeout + non-blocking liveness probe, connection reuse across requests, and idempotency-safe single retry). Header-name casing is not "always copy inbound as-is": for `SourceFormat == codex` requests, every emitted name is finally lowercased to match the real Codex CLI (reqwest/hyper), including proxy-generated headers such as `Host` and `Content-Length`; the Claude path keeps the mixed-case names observed inbound and retains `Connection`, so undici fingerprints are not broken. The Codex path also does not inject a `Connection` header; persistence is handled implicitly by the pool.
- **Optional outbound TLS fingerprint alignment.** On Windows, `schannel-tls` can send Codex-sourced traffic through SChannel so the outbound ClientHello / JA3 matches the local Codex CLI (off by default; ignored on non-Windows; gated to Codex sources only so Claude / Gemini are not mis-fingerprinted). Separately, `claude-ja3-auto-refresh` can capture the local Claude Code ClientHello and apply that Claude JA3 on both the official Anthropic HTTP/2 path and third-party ordered-HTTP/1.1 path; when off, outbound TLS falls back to the default stack and the management panel hides the refresh UI. The SChannel path enables system certificate-chain verification by default.
- **Local client camouflage for management requests.** When the management panel / backend fetches model lists, quota, or quota-reset times without supplying a User-Agent, the proxy fills in a real local client UA: Claude requests get a Claude Code CLI UA that tracks the locally installed Claude Desktop embedded claude-code version; Codex requests get a Codex Desktop UA for desktop-style calls (quota) and a Codex CLI UA for CLI-style calls (model probing). Management fetches of Codex `/models` also rebuild the exact `codex_cli_rs` header set and go out over ordered-HTTP/1.1 (optionally with SChannel), so relays are less likely to recognize a proxy-originated probe. A management endpoint refreshes the cached UAs without restarting.
