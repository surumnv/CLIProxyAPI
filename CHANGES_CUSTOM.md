...
5. **清理会话记录，避免堆积**：claude 会按启动 cwd 在 `~/.claude/projects/<编码名>/` 下写会话
   transcript（`*.jsonl`），这个目录**不在** `~/.claude.json` 的还原范围内，不清理的话每次采集
   都可能留一个、随时间堆积。新增 `removeCaptureTranscriptDir(cwd)`：把临时工作目录的绝对路径
   按 claude 的规则编码（`encodeClaudeProjectDir`：非 ASCII 字母数字的字符全替换为 `-`，已对
   真实条目验证——盘符冒号、路径分隔符、点号、下划线都映射为 `-`），删除对应的
   `~/.claude/projects/<编码名>` 目录。**安全护栏**：仅当目录名含唯一标记 `claude-ja3-capture-`
   时才删（真实项目目录名不可能含此标记，且该标记全为字母数字/连字符、编码后原样保留，算出的
   目标只可能落在采集临时目录上）；删前先 `os.Stat`，不存在则跳过、绝不创建；全程 best-effort，
   任何错误静默忽略。仅在 CPA 自建临时目录时注册此清理，用户显式传 `WorkDir` 时不触发。
