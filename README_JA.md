...
このフォークは、本家に対して以下の変更を加えています。主眼は、実際の Codex / Claude Desktop クライアントから転送されるリクエストを、プロキシによって改変・削除されることなく、本物のクライアントと同じ見た目のまま上流へ届けることにあります。加えて、トランスポートと管理パネルの改善も含みます。機能ごとにまとめると次のとおりです。

- **受信ヘッダーの忠実な透過転送。** 実際の Codex / Claude Desktop クライアントが送るヘッダーを、厳格な許可リストで破棄せず、そのまま保持して上流へ転送します。ただしプロキシが管理すべきヘッダーの権限は引き続き保持します（`Authorization` / `X-Api-Key` はプロバイダー資格情報を運び、上流へ漏洩することはありません。`Anthropic-Beta`、`Accept`、接続・長さ・エンコーディング系ヘッダーはプロキシが管理）。資格情報・セッションに紐づくヘッダー（`Cookie`、`Proxy-Authorization`、`Expect`、`Content-Encoding`）は転送しません。
- **元の HTTP/1.1 ヘッダー順序の保持。** 上流への HTTP/1.1 リクエストは、Go 標準のアルファベット順ではなく、受信クライアントで観測したヘッダー名の順序と大文字小文字を用いて書き出すため、上流は本物のクライアントと一致した順序を受け取ります。keep-alive 接続プールを備えた生の HTTP/1.1 トランスポート（アイドルタイムアウト＋非ブロッキングの生存確認、リクエスト間の接続再利用、冪等性を考慮した 1 回のみのリトライ）で実装しています。
- **管理リクエスト向けのローカル User-Agent 補完。** 管理パネル／バックエンドがモデル一覧・使用量・使用量リセット時刻を User-Agent なしで取得する際、プロキシが実在のローカルクライアント UA を補います。Claude リクエストには、ローカルにインストールされた Claude Desktop 内蔵の claude-code バージョンに追随する Claude Code CLI UA を、Codex リクエストにはデスクトップ系の呼び出し（使用量）には Codex Desktop UA、CLI 系の呼び出し（モデル探索）には Codex CLI UA を付与します。キャッシュされた UA は、再起動せずに管理エンドポイントから更新できます。
