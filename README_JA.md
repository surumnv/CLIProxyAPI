...
このフォークは、本家に対して以下の変更を加えています。主眼は、実際の Codex / Claude Desktop クライアントから転送されるリクエストを、プロキシによって改変・削除されることなく、本物のクライアントと同じ見た目のまま上流へ届けることにあります。HTTP ヘッダー、HTTP/1.1 のワイヤフォーマット、TLS 指紋の 3 層で可能な限り本物のクライアントへ揃え、加えて管理パネルとプロバイダー設定の改善も含みます。機能ごとにまとめると次のとおりです。

- **受信ヘッダーと重要フィールドの忠実な透過転送。** 実際の Codex / Claude Desktop クライアントが送るヘッダーを、厳格な許可リストで破棄せず、そのまま保持して上流へ転送します。ただしプロキシが管理すべきヘッダーの権限は引き続き保持します（`Authorization` / `X-Api-Key` はプロバイダー資格情報を運び、上流へ漏洩することはありません。`Anthropic-Beta`、`Accept`、接続・長さ・エンコーディング系ヘッダーはプロキシが管理）。資格情報・セッションに紐づくヘッダー（`Cookie`、`Proxy-Authorization`、`Expect`、`Content-Encoding`）は転送しません。Claude 上流にはクライアントが送った `temperature` を残し、サードパーティの Claude base には公式 OAuth 専用 beta を誤注入しません。Responses リクエストが Chat Completions に変換される場合は、Responses-Lite 専用ヘッダーを剥がし、プロトコル不一致による上流拒否を避けます。
- **元の HTTP/1.1 ヘッダー順序を保持し、ヘッダー名の大小文字はソースごとに決める。** 上流への HTTP/1.1 リクエストは、まず受信クライアントで観測したヘッダーの**順序**で書き出され、Go 標準のアルファベット順にはしません。この能力は Codex / Claude の主経路に加え、OpenAI 互換プロバイダー経由の転送経路にも適用されます。keep-alive 接続プールを備えた生の HTTP/1.1 トランスポート（アイドルタイムアウト＋非ブロッキングの生存確認、リクエスト間の接続再利用、冪等性を考慮した 1 回のみのリトライ）で実装しています。ヘッダー名の大小文字は「常に受信どおり」ではありません。`SourceFormat == codex` のリクエストでは、最終的にすべてのヘッダー名を小文字へ統一します（本物の Codex CLI / reqwest / hyper に合わせ、プロキシ自身が生成する `Host` や `Content-Length` も含む）。Claude 経路では受信時に観測した mixed-case を使い、`Connection` も残すため undici の指紋を壊しません。Codex 経路では `Connection` ヘッダーも注入せず、永続接続は接続プールが暗黙に維持します。
- **上流 TLS 指紋の整合（任意）。** Windows では `schannel-tls` により Codex ソースの上流通信を SChannel 経由にし、ClientHello / JA3 をローカルの Codex CLI と一致させられます（既定オフ、非 Windows では無視、Codex ソースのみに限定して Claude / Gemini を誤フィンガープリントしません）。また `claude-ja3-auto-refresh` により、ローカル Claude Code の ClientHello を収集し、公式 Anthropic HTTP/2 経路とサードパーティ ordered-HTTP/1.1 経路で Claude の JA3 を揃えられます。オフ時は既定 TLS に戻り、管理パネルの更新 UI も非表示になります。SChannel 経路は既定でシステム証明書チェーン検証を有効にします。
- **管理リクエスト向けのローカルクライアント偽装。** 管理パネル／バックエンドがモデル一覧・使用量・使用量リセット時刻を User-Agent なしで取得する際、プロキシが実在のローカルクライアント UA を補います。Claude リクエストには、ローカルにインストールされた Claude Desktop 内蔵の claude-code バージョンに追随する Claude Code CLI UA を、Codex リクエストにはデスクトップ系の呼び出し（使用量）には Codex Desktop UA、CLI 系の呼び出し（モデル探索）には Codex CLI UA を付与します。管理面からの Codex `/models` 取得では、実際の `codex_cli_rs` のヘッダー集合へ再構成し、ordered-HTTP/1.1（任意で SChannel 併用）で上流へ送るため、中継サーバーにプロキシ由来の探査と判別されにくくします。キャッシュされた UA は、再起動せずに管理エンドポイントから更新できます。
- **Claude Desktop 起動時プローブのローカル短絡応答。** Claude Desktop が起動時に送る極小の可用性プローブ（および `HEAD /` の到達性チェック）をプロキシが認識してローカルで応答するため、上流へ転送されたり中継サーバーに拒否されたりしません。判定条件は非常に狭く、認証後に実行されるため、通常の会話が該当することはありません。
- **より多くのプロバイダー種別で任意の表示名。** Gemini / Codex / Claude / Vertex の資格情報に、管理パネルで任意の表示名を付けられます（従来は OpenAI 互換プロバイダーのみ）。この名前は表示ラベルにすぎず、資格情報の照合・重複排除・削除には一切使用しません。

詳細とアップグレード時の再適用手順は `CHANGES_CUSTOM.md` に記載しています。
