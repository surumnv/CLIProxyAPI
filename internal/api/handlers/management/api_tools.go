...
// select for this credential, following the same precedence (per-request
// proxy_url → credential proxy_url → per-API-key config proxy → global
// proxy-url) and the same build-validation. It is used to key/dial the
// ordered-HTTP/1.1 transport on the Codex models path; an empty result means a
// direct connection.
func (h *Handler) resolveAPICallProxyURL(auth *coreauth.Auth, requestProxyURL string) string {
...
func (h *Handler) APICall(c *gin.Context) {
	...
	if isCodexModelsFetch(method, parsedURL, req.Header) {
		...
		proxyURL := h.resolveAPICallProxyURL(auth, requestProxyURL)
