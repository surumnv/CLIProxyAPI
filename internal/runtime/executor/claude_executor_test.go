...
func TestApplyClaudeHeaders_UsesLocalClaudeUAForNonClaudeIncomingHeaders(t *testing.T) {
	...
	if err := applyClaudeHeaders(req, auth, "key-local-ua-incoming-headers", false, nil, nil, cfg, incoming, false); err != nil {
...
func TestApplyClaudeHeaders_PreservesClaudeIncomingUAFromExplicitHeaders(t *testing.T) {
	...
	if err := applyClaudeHeaders(req, auth, "key-claude-ua-incoming-headers", false, nil, nil, nil, incoming, false); err != nil {
		t.Fatalf("applyClaudeHeaders() error = %v", err)
	}
...
func TestClaudeExecutor_NonClaudeRequestUsesClaudeCode220CLIFingerprint(t *testing.T) {
	...
	executor := NewClaudeExecutor(&config.Config{ClaudeHeaderDefaults: config.ClaudeHeaderDefaults{OS: "MacOS", Arch: "arm64"}})
	auth := &cliproxyauth.Auth{Attributes: map[string]string{
...
// Fork behavior (commit dc0f7f9b): with thinking inactive a translated or
// cloaked caller keeps every sampling knob it sent. Anthropic only rejects
// top_p/top_k while thinking is active, so nothing is stripped here.
