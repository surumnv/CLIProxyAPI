...
type utlsRoundTripper struct {
	...
	// useClaudeFingerprint makes this transport reproduce the captured Claude
	// Code ClientHello (JA3) instead of the Chrome preset. It is set only for
	// the Anthropic HTTP/2 path when claude-ja3-auto-refresh is on and a capture
	// exists; every other host keeps the Chrome fingerprint.
...
// newClaudeFingerprintUtlsRoundTripper builds the HTTP/2 utls transport that
// replays the captured Claude Code ClientHello on official Anthropic hosts.
func newClaudeFingerprintUtlsRoundTripper(proxyURL string) *utlsRoundTripper {
	roundTripper := newUtlsRoundTripper(proxyURL)
	roundTripper.useClaudeFingerprint = true
	return roundTripper
}
...
func (t *utlsRoundTripper) createConnection(ctx context.Context, host, addr string) (*http2.ClientConn, error) {
	...
	// A fresh spec is required per connection because ApplyPreset mutates it.
	// When no capture is stored, fall back to the Chrome preset.
		...
		if spec := fingerprint.ClaudeSpecH2(); spec != nil {
			...
			if errPreset := tlsConn.ApplyPreset(spec); errPreset != nil {
				if errClose := conn.Close(); errClose != nil {
					return nil, fmt.Errorf("utls: apply captured Claude ClientHello: %w; close connection: %v", errPreset, errClose)
				}
				return nil, fmt.Errorf("utls: apply captured Claude ClientHello: %w", errPreset)
			}
		}
	}
...
func NewUtlsHTTPClient(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, timeout time.Duration) *http.Client {
	...
	// SChannel provider (matching the Codex CLI JA3) is decided per request from
	// the context (executor.WithSChannelTLS), set only for Codex-originated
	...
	// Claude fingerprint alignment is decided per client from the request context
	// (executor.WithClaudeFingerprint), set only for Claude-originated requests
	// when claude-ja3-auto-refresh is on. With the toggle on, the captured
	// Claude ClientHello is replayed over HTTP/2; with it off, the built-in
	// Claude Code Node/OpenSSL profile is used.
	var anthropicRT http.RoundTripper
	if cliproxyexecutor.ClaudeFingerprintFromContext(ctx) {
		anthropicRT = newClaudeFingerprintUtlsRoundTripper(proxyURL)
	} else {
	...
	if proxyURL != "" {
		...
		wrapOrderedTransport = false
	}
	if wrapOrderedTransport {
		standardTransport = sharedOrderedH1RoundTripper(orderedProxyURL, standardTransport)
	}
