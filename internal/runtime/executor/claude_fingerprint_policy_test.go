...
func TestClaudeExecutor_OfficialAPIKeyDefaultRespectsClient(t *testing.T) {
	...
	// Fork behavior (commit de020b03): a caller announcing a non-Claude client is
	// presented upstream as the locally installed Claude Code CLI, so the third
	// party client name never reaches a Claude relay. Everything else about
	// caller-owned mode (betas, tools, no CLI diagnostics) stays caller-owned.
	if got := claudeFingerprintHeaderValue(seenHeaders, "User-Agent"); got != misc.LocalClaudeCodeUserAgent() {
...
func TestKimiExecutor_ClaudeMessagesWithAndWithoutClaudeCodeCLIFingerprint(t *testing.T) {
	...
	// Fork behavior (commit de020b03): the caller's non-Claude UA is replaced by the
	// local Claude Code CLI identity here too; the caller's betas still pass through.
	if got := claudeFingerprintHeaderValue(seenHeaders[0], "User-Agent"); got != misc.LocalClaudeCodeUserAgent() {
		t.Fatalf("default Kimi User-Agent = %q, want local Claude UA %q", got, misc.LocalClaudeCodeUserAgent())
	}
...
// Fork behavior (commit de020b03): a caller that does send a User-Agent still
// does not get it forwarded verbatim when it names a non-Claude client. The
// request reaches the upstream as the locally installed Claude Code CLI.
func TestClaudeExecutor_CallerOwnedRewritesNonClaudeUserAgent(t *testing.T) {
	var seen http.Header
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		seen = r.Header.Clone()
		w.Header().Set("Content-Type", "application/json")
	...
	if got := seen.Get("User-Agent"); got != misc.LocalClaudeCodeUserAgent() {
		t.Fatalf("User-Agent = %q, want local Claude UA %q", got, misc.LocalClaudeCodeUserAgent())
	}
}
